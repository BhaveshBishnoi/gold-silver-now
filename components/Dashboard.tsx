...
const StatCard = ({ title, price, change, sparkData, color, type, currency, currencySym, unitLabel, format }: StatCardProps) => {
    ...
    return (
        <Card className={cn(
            "relative overflow-hidden border-none shadow-[0_15px_35px_-12px_rgba(0,0,0,0.06)] transition-all duration-500 hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.1)] hover:-translate-y-1 group rounded-[2rem]",
            isGold
                ? "bg-gradient-to-br from-white via-orange-50/30 to-amber-50/50"
                : "bg-gradient-to-br from-white via-slate-50/30 to-slate-100/50"
        )}>
            {/* Background Decor */}
            <div className={cn(
                "absolute -top-24 -right-24 w-64 h-64 rounded-full blur-[100px] opacity-10 transition-opacity duration-500 group-hover:opacity-20",
            ...
            <CardContent className="p-7 md:p-9 relative">
                <div className="flex justify-between items-start mb-6 md:mb-8">
                    <div className="flex flex-col">
                        <h3 className={cn(
                            "text-lg md:text-xl font-black tracking-tight flex items-center gap-2.5",
                        ...
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1.5 opacity-80">
                            Live Spot Price
                        </span>
                    </div>

                    <div className={cn(
                        "flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black shadow-sm border backdrop-blur-md",
                        isPositive
                            ? "bg-green-50/70 text-green-700 border-green-100/40"
                            : "bg-red-50/70 text-red-700 border-red-100/40"
                    )}>
                ...
                <div className="space-y-1 mb-8 md:mb-10">
                    <div className="flex items-baseline gap-2.5">
                        <span className="text-2xl md:text-3xl font-bold text-slate-300 select-none tracking-tighter">{currency}</span>
                        <span className={cn(
                            "text-5xl md:text-6xl lg:text-7xl font-black tracking-tighter tabular-nums leading-none",
                            isGold ? "text-slate-950" : "text-slate-900"
                        )}>
                            {format(price).replace(currencySym, '')}
                        </span>
                    </div>
                    <p className="text-[11px] md:text-xs font-bold text-slate-400 flex items-center gap-2 opacity-80">
                        Unit: {unitLabel} <span className="w-1 h-1 rounded-full bg-slate-200" /> Market Open
                    </p>
                </div>
